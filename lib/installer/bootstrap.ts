/**
 * Bootstrap da instância MindZap.
 * Configura settings iniciais após migrations.
 *
 * MindZap usa MASTER_PASSWORD para auth, não Supabase Auth.
 ...
 * MindZap usa MASTER_PASSWORD via env var, não Supabase Auth.
...
export async function bootstrapInstance({
  ...
  const companyName = adminName?.trim() || emailNorm.split('@')[0] || 'MindZap';
